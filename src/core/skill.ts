...
function hostRulesBlock(host: string): string {
  ...
  return [
    ...
    "3. During the session, call `memory-brain session-check --text \"<latest_turn_text>\" --session \"<session_id>\" --workspace \"<cwd>\"` after each turn. It enforces a mandatory memory check every 2 turns and writes only on hit.",
    ...
    "- The user uses stability cues such as \"默认\", \"以后\", \"长期\", \"下次继续\", \"remember\", \"default\", or \"always\" while stating a preference or constraint.",
    ...
    "- The periodic every-2-turn check finds durable candidates worth persisting.",
    "",
    "Write rules:",
